...
def init_cities(n):
    res = []
    a1 = random.sample(range(1,2*n+1),n)
    a2 = random.sample(range(1,2*n+1),n)
...
def draw_H(citys, best_gens):
    ...
    for (from_, to_) in H_path:
        ...
        ax.annotate(s=str(to_+1), xy=citys[to_], xytext=(-8, -4), textcoords='offset points', fontsize=20)
    ax.axis('equal')
    ax.grid()
    plt.show()
if __name__ == '__main__':
    max_evolution_num = 20000  # 最大进化代数
    population_num = 100  # 种群数目
    cross_pro = 0.6  # 交叉概率
    mutation_pro = 0.1  # 变异概率
    N_city = 25 #城市数
    #citys = [(2, 6), (2, 4), (1, 3), (4, 6), (5, 5), (4, 4), (6, 4), (3, 2)]  # 城市坐标
    #citys = init_cities(N_city)
    citys = [(47, 47), (33, 4), (37, 12), (10, 15), (21, 38), (2, 10), (20, 36), (25, 49), (27, 43), (40, 20), (22, 48), (36, 24), (39, 17), (48, 44), (14, 11), (4, 27), (45, 41), (19, 19), (32, 14), (50, 16), (28, 8), (6, 30), (34, 34), (23, 2), (26, 7)]
    ...
    for step in range(max_evolution_num):
        ...
        chroms = reverse(citys, chroms)  # 逆转操作，单个染色体变得更加优秀
        ...
        distance_mem.append(min(distance_arr[best_gens_idx] , min_distance))
        if distance_arr[best_gens_idx] < min_distance:
            ...
            best_fit_index = step+1  # 更新最短路径出现的代数
    print('通过{}代的基因进化，精英染色体出现在第{}代，基因序列为：'.format(max_evolution_num, best_fit_index))
    [print(str(v+1), end=',' if i < len(best_gens) - 1 else '\n') for i, v in enumerate(best_gens)]
    print('精英染色体映射的最短路径为：{}'.format(round(min_distance,2)))
    draw_H(citys, best_gens)
    
    plt.plot(range(1,len(distance_mem)+1),distance_mem)
